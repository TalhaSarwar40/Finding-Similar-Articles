            ...
            for j in range(0,len(tagged_token),1):
                if tagged_token[j][1] == 'NN' or 'NNS' or 'NNP' or 'NNPS':
                    ...
                    if len(tagged_token[j][0]) > 2:
                        ...
                        #print(tagged_token[j][0])
                        #print(Noun_word.count(tagged_token[j][0]))
                        if Noun_word.count(tagged_token[j][0]) > 10:
                            ...
                            if tuple((tagged_token[j][0], Noun_word.count(tagged_token[j][0]))) not in NounTF_tuple_list:
                                ...
                                #print("NTL"+str(NounTF_tuple_list))
...
class CosineSimilarityWithMU:
    ...
    def BestCosineSimilarityWithMU(self,paper_name,Keyphrases,root_keyphrase_list,reference_cosine_list):
        ...
        sum_sqr_2 = 0
        cosine_similarity = 0
        #print("Root Keyphrases: "+str(root_keyphrase_list))
        #print("Reference Keyphrases: "+ str(Keyphrases))
        ...
        for i in range(0,len(root_keyphrase_list),1):
            for j in range(0,len(Keyphrases),1):
                if root_keyphrase_list[i][0].strip() == Keyphrases[j][0].strip():
                    #print(root_keyphrase_list[i][0])
                    #print(Keyphrases[j][0])
                    #print(i)
...
class JaccardSimilarity:
    ...
    def BestJaccardSimilarity(self,paper_name,Keyphrases,list_1,reference_jaccard_list):
        ...
        #.strip()
        #print("in jaccard: "+str(len(Keyphrases)))
...
root_paper = open(main_article_path, "r",encoding="utf-8")

root_paper_read = root_paper.read()

#print(root_paper_read)
...
MainKeyphrases = treemanager.ReturnKeyPhrases (final_node_list, candidate_phrases_count_3,text)    
print(MainKeyphrases)
           
root_score_list = cs.Root(MainKeyphrases)
root_keyphrase_list = js.Root(MainKeyphrases)


#print("Root paper keyphrases: ")
#print(Keyphrases)
...
for i in range(0,len(full_path),1):
   ...
   # print("..................")
    ...
    with open(full_path[i], "r",encoding="utf-8") as f:
        ...
        print("Main Keyphrase: ")
        print(MainKeyphrases)
        print("")
        print("Similar Keyphrase: ")
        print(Keyphrases)
        print("...............................")
        ...
        #word2Vec = cosineSimilarityWord2Vec(paper_name, MainKeyphrases, Keyphrases)
        #simWord2VecList.append(word2Vec)
...
#print(simWord2VecList)
simWord2VecList.sort(key=lambda y: y[1], reverse = True)
#simWord2VecList.sort(reverse = True)
#print(simWord2VecList)
...
for j in range(0,len(reference_cosine_list),1):
    ...
    if reference_cosine_list[j].SimilarityScore >=0:
        print("Main Article Name: " + str(main_paper_name))
        print("Similar Article Name: "+str(reference_cosine_list[j].PaperName))
        print("Cosine Similarity Score: "+str(reference_cosine_list[j].SimilarityScore))
        #print("paper number: "+str(j))
        print("\n")
    else:
        break
'''
...
for j in range(0,len(reference_jaccard_list),1):
    if reference_jaccard_list[j].SimilarityScore >=0:
        print("Main Article Name: " + str(main_paper_name))
        print("Similar Article Name: "+str(reference_jaccard_list[j].PaperName))
